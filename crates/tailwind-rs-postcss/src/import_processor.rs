...
impl ImportProcessor {
    ...
    pub fn process_imports(&mut self, css: &str, base_path: &str) -> Result<String, ImportError> {
        ...
        for line in css.lines() {
            if line.trim().starts_with("@import") {
                ...
                let resolved_path = self
                    .resolver
                    .resolve(&import_statement.import_path, base_path)?;

                // Check for circular dependencies
                if let Err(circular) = self
                    .dependency_graph
                    .add_dependency(base_path, &resolved_path)
                {
                ...
                let content = self
                    .resolver
                    .get_import_content(&import_statement.import_path, base_path)?;
        ...
        // Cache the result
        self.cache
            .cache_content(base_path.to_string(), processed_css.clone());

        Ok(processed_css)
    }

    /// Process imports with advanced options
    pub fn process_imports_advanced(
        &mut self,
        css: &str,
        ...
        for line in css.lines() {
            if line.trim().starts_with("@import") {
                ...
                let resolved_path = self
                    .resolver
                    .resolve(&import_statement.import_path, &options.search_paths[0])?;

                // Check for circular dependencies
                if let Err(circular) = self
                    .dependency_graph
                    .add_dependency(&options.search_paths[0], &resolved_path)
                {
                ...
                let content = self
                    .resolver
                    .get_import_content(&import_statement.import_path, &options.search_paths[0])?;
    ...
    fn parse_import_statement(&self, line: &str) -> Result<ImportStatement, ImportError> {
        ...
        if let Some(cap) = import_pattern.captures(line) {
            ...
            Err(ImportError::InvalidImportStatement {
                line: line.to_string(),
            })
        }
    }
...
impl ImportResolver {
    ...
    pub fn resolve(&self, import_path: &str, base_path: &str) -> Result<String, ImportError> {
        ...
        let base_dir =
            Path::new(base_path)
                .parent()
                .ok_or_else(|| ImportError::InvalidBasePath {
        ...
        Err(ImportError::ImportNotFound {
    ...
    /// Get import content
    pub fn get_import_content(
        &self,
        import_path: &str,
        base_path: &str,
    ) -> Result<String, ImportError> {
        let resolved_path = self.resolve(import_path, base_path)?;
        std::fs::read_to_string(&resolved_path).map_err(|_| ImportError::FileReadError {
            path: resolved_path,
        })
    }
}
...
mod tests {
    ...
    fn test_import_statement_parsing() {
        ...
        assert_eq!(
            statement.media_query,
            Some("screen and (max-width: 768px)".to_string())
        );
    }
