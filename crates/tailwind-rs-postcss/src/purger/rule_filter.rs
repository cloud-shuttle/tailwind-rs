...
impl RuleFilter {
    ...
    pub fn filter_rules(&self, css: &str, used_classes: &HashSet<String>, config: &PurgeConfig) -> Result<String, PurgeError> {
    ...
    pub fn filter_rules_advanced(&self, css: &str, used_classes: &HashSet<String>, config: &PurgeConfig, options: &PurgeOptions) -> Result<String, PurgeError> {
    ...
    fn should_keep_rule(&self, selector: &str, used_classes: &HashSet<String>, config: &PurgeConfig) -> bool {
        // Check safelist
        if config.safelist.iter().any(|pattern| selector.contains(pattern)) {
            return true;
        }
        
        // Check blocklist
        if config.blocklist.iter().any(|pattern| selector.contains(pattern)) {
            return false;
        }
        ...
        selector_classes.iter().any(|class| used_classes.contains(class))
    }
