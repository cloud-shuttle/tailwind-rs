...
use tailwind_rs_core::css_generator::parsers::UtilityParser;
...
fn test_broken_classes() {
    ...
    // First test non-variant classes that work
    let working_classes = vec![
        "animate-spin",
        "scale-105",
        "rotate-45",
    ...
    println!("Testing {} working non-variant classes:", working_classes.len());
    for class in working_classes {
        match generator.add_class(class) {
            Ok(_) => {
                println!("  ✅ {} - Added successfully", class);
    ...
    // Now test variant classes that fail
    ...
    println!("\nTesting {} variant classes that fail:", variant_classes.len());
    for class in variant_classes {
        println!("  🔍 Testing variant class: {}", class);
        match generator.add_class(class) {
            Ok(_) => {
                println!("    ✅ {} - Added successfully", class);
            },
