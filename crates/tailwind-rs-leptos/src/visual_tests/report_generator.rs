...
impl VisualTestReport {
    ...
    pub fn get_failed_tests(&self) -> Vec<&VisualComparison> {
        self.test_results.iter().filter(|r| !r.classes_match).collect()
    }
    ...
    pub fn format_report(&self) -> String {
        ...
        if !self.failed_tests == 0 {
            ...
            for (i, failed_test) in self.get_failed_tests().iter().enumerate() {
                output.push_str(&format!("  {}. {} - Classes don't match\n", i + 1, failed_test.component_name));
...
mod tests {
    ...
    fn test_merge_reports() {
        ...
        let mut report1 = VisualTestReport::new(vec![comparison1], std::time::Duration::from_secs(1));
