...
impl BackgroundPropertiesParser {
    ...
    fn parse_bg_size_class(&self, class: &str) -> Option<Vec<CssProperty>> {
        if let Some(value) = class.strip_prefix("bg-size-[") {
            if let Some(value) = value.strip_suffix("]") {
                return Some(vec![CssProperty { name: "background-size".to_string(), value: value.to_string(), important: false }]);
            }
        }
        None
    }
    ...
    fn parse_bg_position_class(&self, class: &str) -> Option<Vec<CssProperty>> {
        if let Some(value) = class.strip_prefix("bg-position-[") {
            if let Some(value) = value.strip_suffix("]") {
                return Some(vec![CssProperty { name: "background-position".to_string(), value: value.to_string(), important: false }]);
            }
        }
        None
    }
...
impl UtilityParser for BackgroundPropertiesParser {
    ...
    fn get_supported_patterns(&self) -> Vec<&'static str> {
        vec![
            "bg-size-[*]", "bg-position-[*]", "bg-no-repeat", "bg-repeat", "bg-repeat-x", "bg-repeat-y",
            "bg-repeat-round", "bg-repeat-space", "bg-fixed", "bg-local", "bg-scroll",
