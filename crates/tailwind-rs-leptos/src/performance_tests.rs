...
impl LeptosPerformanceBenchmarks {
    ...
    pub fn run_all_benchmarks() -> BenchmarkResults {
        ...
        let (class_gen_total, class_gen_avg) =
            PerformanceTestUtils::benchmark_class_generation(iterations);
        let (dynamic_total, dynamic_avg) =
            PerformanceTestUtils::benchmark_dynamic_class_builder(iterations);
        let (signal_total, signal_avg) = PerformanceTestUtils::benchmark_signal_manager(iterations);
        let (complex_total, complex_avg) =
            PerformanceTestUtils::benchmark_complex_class_generation(iterations);
    ...
    pub fn run_regression_tests() -> RegressionTestResults {
        ...
        let (_class_gen_total, class_gen_avg) =
            PerformanceTestUtils::benchmark_class_generation(iterations);
        let (_dynamic_total, dynamic_avg) =
            PerformanceTestUtils::benchmark_dynamic_class_builder(iterations);
        let (_signal_total, signal_avg) =
            PerformanceTestUtils::benchmark_signal_manager(iterations);
...
mod tests {
    ...
    fn test_performance_utils_average() {
        let iterations = 5;
        let (results, total_duration, average_duration) =
            PerformanceTestUtils::measure_average_time(
                || {
                    // Simple operation
                    let mut sum = 0;
                    for i in 0..100 {
    ...
    fn test_class_generation_benchmark() {
        let iterations = 100;
        let (total_duration, average_duration) =
            PerformanceTestUtils::benchmark_class_generation(iterations);
    ...
    fn test_memory_usage_benchmark() {
        ...
        let (total_duration, average_duration) =
            PerformanceTestUtils::benchmark_memory_usage(iterations);
    ...
    fn test_regression_tests() {
        ...
        assert!(
            results.class_generation,
            "Class generation performance regression: {:?} > {:?}",
            results.class_generation_time, results.thresholds.class_generation
        );

        assert!(
            results.dynamic_class_builder,
            "Dynamic class builder performance regression: {:?} > {:?}",
            results.dynamic_class_builder_time, results.thresholds.dynamic_class_builder
        );

        assert!(
            results.signal_manager,
            "Signal manager performance regression: {:?} > {:?}",
            results.signal_manager_time, results.thresholds.signal_manager
        );
    }
