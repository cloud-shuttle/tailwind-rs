...
fn test_css_generator_api_contract_invalid_input() {
    ...
    // Should still generate CSS (generator has default content)
    assert!(!css.is_empty());
}
...
fn test_error_handling() {
    // Test with invalid class names (should not panic)
    let mut generator = CssGenerator::new();
    generator.add_class("invalid-class-name").unwrap();
    generator.add_class("another-invalid-class").unwrap();
    
    let css = generator.generate_css();
    
    // Should still generate CSS (invalid classes are ignored)
    assert!(!css.is_empty());
}
...
fn test_performance_with_many_classes() {
    ...
    for i in 0..100 {
        ...
        generator.add_class(&format!("m-{}", i)).unwrap();
        generator.add_class(&format!("text-{}", i)).unwrap();
    }
