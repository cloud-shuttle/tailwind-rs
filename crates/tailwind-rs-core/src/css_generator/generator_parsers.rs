...
impl CssGeneratorParsers for super::core::CssGenerator {
    fn class_to_properties(&self, _class: &str) -> Result<Vec<CssProperty>> {
        // Core CssGenerator is simple - just return empty for now
        Ok(vec![])
    }

    fn parse_variants(&self, class: &str) -> (Vec<String>, String) {
        // Simple variant parsing for core
...
impl CssGeneratorParsers for super::generator::CssGenerator {
    fn class_to_properties(&self, class: &str) -> Result<Vec<CssProperty>> {
        // Use the parser trie to find the appropriate parser
        self.parser_trie.parse_class(class).ok_or_else(|| {
            TailwindError::class_generation(format!("Unknown class: {}", class))
        })
    }
