...
pub use test_cases::{ComponentTestCase, ResponsiveTestCase, StateTestCase, ThemeTestCase};
pub use test_runner::{VisualTestConfig, VisualTestRunner};
pub use test_utils::{VisualComparison, VisualDifference, VisualSnapshot, VisualTestUtils};
...
impl VisualRegressionTests {
    ...
    pub fn run_component_consistency_tests() -> Vec<VisualComparison> {
        ...
        test_cases
            .into_iter()
            .map(|test_case| {
                let snapshot1 = VisualTestUtils::capture_component_state(
                    &test_case.component_name,
                    &test_case.expected_classes,
                );
                let snapshot2 = VisualTestUtils::capture_component_state(
                    &test_case.component_name,
                    &test_case.expected_classes,
                );
