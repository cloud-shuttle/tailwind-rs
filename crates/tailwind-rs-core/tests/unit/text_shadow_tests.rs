...
mod text_shadow_integration_tests {
    use super::*;
    use tailwind_rs_core::utilities::*;
    ...
    fn test_text_shadow_responsive() {
        let builder = ClassBuilder::new()
            .text_shadow_sm()
            .responsive(crate::responsive::Breakpoint::Md, "text-shadow-lg");
        
        let class_set = builder.build();
        assert!(class_set.classes.contains("text-shadow-sm"));
        assert!(class_set.responsive.contains_key(&crate::responsive::Breakpoint::Md));
        assert!(class_set.responsive.get(&crate::responsive::Breakpoint::Md).unwrap().contains("text-shadow-lg"));
    }
