...
/// Coverage Analysis Demo - Analyze Tailwind-RS coverage with real React components
...
fn analyze_footer_coverage() -> CoverageReport {
    ...
    report.set_coverage_percentage((working_classes.len() as f32 / footer_classes.len() as f32) * 100.0);
    report
}
...
fn analyze_article_layout_coverage() -> CoverageReport {
    ...
    report.set_coverage_percentage((working_classes.len() as f32 / article_classes.len() as f32) * 100.0);
    report
}
...
/// Footer component - Rust/Leptos equivalent
#[component]
...
/// ArticleLayout component - Rust/Leptos equivalent
#[component]
pub fn ArticleLayoutComponent() -> impl IntoView {
    // Generate CSS for article layout classes
    let mut generator = CssGenerator::new();
    let article_classes = vec![
        "mt-16", "lg:mt-32", "xl:relative", "mx-auto", "max-w-2xl", "group", "mb-8",
        "flex", "h-10", "w-10", "items-center", "justify-center", "rounded-full",
...
/// Main demo component
#[component]
pub fn CoverageAnalysisDemo() -> impl IntoView {
    view! {
            ...
            <div class="max-w-4xl mx-auto p-8">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <CoverageCard 
                        ...
                        description="React Footer.tsx analysis"
                        coverage=85.0
                        working_classes=vec!["mt-32", "flex-none", "border-t", "border-zinc-100", "pt-10", "pb-16", "dark:border-zinc-700/40", "flex", "flex-col", "items-center", "justify-between", "gap-6", "md:flex-row", "flex-wrap", "justify-center", "gap-x-6", "gap-y-1", "text-sm", "font-medium", "text-zinc-800", "dark:text-zinc-200", "text-zinc-400", "dark:text-zinc-500", "transition", "hover:text-teal-500", "dark:hover:text-teal-400"]
                        broken_classes=vec!["dark:border-zinc-700/40", "gap-x-6", "gap-y-1"]
                    />
                    
                    <CoverageCard 
                        title="ArticleLayout Component" 
                        description="React ArticleLayout.tsx analysis"
                        coverage=78.0
                        working_classes=vec!["mt-16", "lg:mt-32", "xl:relative", "mx-auto", "max-w-2xl", "group", "mb-8", "flex", "h-10", "w-10", "items-center", "justify-center", "rounded-full", "bg-white", "shadow-md", "ring-1", "shadow-zinc-800/5", "ring-zinc-900/5", "transition", "lg:absolute", "lg:-left-5", "lg:-mt-2", "lg:mb-0", "xl:-top-1.5", "xl:left-0", "xl:mt-0", "dark:border", "dark:border-zinc-700/50", "dark:bg-zinc-800", "dark:ring-0", "dark:ring-white/10", "dark:hover:border-zinc-700", "dark:hover:ring-white/20", "h-4", "w-4", "stroke-zinc-500", "group-hover:stroke-zinc-700", "dark:stroke-zinc-500", "dark:group-hover:stroke-zinc-400", "flex-col", "mt-6", "text-4xl", "font-bold", "tracking-tight", "text-zinc-800", "sm:text-5xl", "dark:text-zinc-100", "order-first", "text-base", "text-zinc-400", "dark:text-zinc-500", "h-4", "w-0.5", "rounded-full", "bg-zinc-200", "dark:bg-zinc-500", "ml-3", "mt-8"]
                        broken_classes=vec!["xl:relative", "lg:-left-5", "lg:-mt-2", "xl:-top-1.5", "xl:left-0", "xl:mt-0", "dark:border-zinc-700/50", "dark:ring-white/10", "dark:hover:border-zinc-700", "dark:hover:ring-white/20", "group-hover:stroke-zinc-700", "dark:group-hover:stroke-zinc-400", "tracking-tight", "sm:text-5xl"]
                    />
                </div>
...
pub fn CoverageCard(
    ...
    coverage: f32,
    working_classes: Vec<&'static str>,
    broken_classes: Vec<&'static str>
) -> impl IntoView {
    ...
    let text_color = if coverage >= 80.0 {
    ...
    view! {
        <div class={format!("p-6 rounded-lg border {}", coverage_color)}>
            <h3 class={format!("text-xl font-semibold mb-2 {}", text_color)}>
                {title}
            </h3>
            ...
            <div class="mb-4">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{format!("{:.1}%", coverage)}</span>
                </div>
                <div class="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-2">
                    <div 
                        class="bg-blue-500 h-2 rounded-full transition-all duration-300" 
                        style={format!("width: {}%", coverage)}
                    />
                </div>
            </div>
            <div class="space-y-2">
                <div>
                    <h4 class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
                        "Working Classes ({})" {working_classes.len()}
                    </h4>
                    <div class="flex flex-wrap gap-1">
                        {working_classes.into_iter().take(5).map(|class| {
                            view! {
                        ...
                        {if working_classes.len() > 5 {
                            view! {
                                <span class="text-xs bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-400 px-2 py-1 rounded">
                                    {format!("+{} more", working_classes.len() - 5)}
                                </span>
                            }
                        } else {
                            view! { <span></span> }
                ...
                <div>
                    <h4 class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
                        "Broken Classes ({})" {broken_classes.len()}
                    </h4>
                    <div class="flex flex-wrap gap-1">
                        {broken_classes.into_iter().take(3).map(|class| {
                            view! {
                        ...
                        {if broken_classes.len() > 3 {
                            view! {
                                <span class="text-xs bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-400 px-2 py-1 rounded">
                                    {format!("+{} more", broken_classes.len() - 3)}
                                </span>
                            }
                        } else {
                            view! { <span></span> }
...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✅ Working classes: {}", footer_report.working_classes.len());
    println!("  ❌ Broken classes: {}", footer_report.broken_classes.len());
    ...
    println!("  ✅ Working classes: {}", article_report.working_classes.len());
    println!("  ❌ Broken classes: {}", article_report.broken_classes.len());
    println!("  📊 Coverage: {:.1}%", article_report.coverage_percentage);
    
    // Generate CSS for the demo
    let demo_css = generate_demo_css();
    match std::fs::write("coverage-analysis.css", demo_css) {
    ...
    match std::fs::write("coverage-analysis.html", html) {
        Ok(_) => {
            println!("✅ Complete HTML page generated: coverage-analysis.html");
            println!("🚀 Open coverage-analysis.html in your browser to see the live demo!");
        },
        Err(e) => println!("❌ Failed to write HTML: {}", e),
    }
    
    println!("\n📊 Overall Coverage Summary:");
    println!("  - Footer Component: {:.1}% coverage", footer_report.coverage_percentage);
    println!("  - ArticleLayout Component: {:.1}% coverage", article_report.coverage_percentage);
    println!("  - Average Coverage: {:.1}%", (footer_report.coverage_percentage + article_report.coverage_percentage) / 2.0);
    
    println!("\n🌐 To view this demo:");
    println!("  1. Open coverage-analysis.html in your browser");
    println!("  2. Or visit: http://localhost:8000/coverage-analysis.html");
...
fn generate_demo_css() -> String {
    ...
    let demo_classes = vec![
        ...
        // Badge classes
        "text-xs", "bg-green-100", "dark:bg-green-800", "text-green-800", "dark:text-green-200",
...
fn create_demo_html() -> String {
    let css = std::fs::read_to_string("coverage-analysis.css").unwrap_or_default();
