...
impl TailwindPlugin for AnimationPlugin {
    ...
    fn init(&mut self) -> Result<()> {
        ...
        self.animations.insert("fade-in".to_string(), 
...
fn test_plugin_custom_animations() {
    ...
    assert_eq!(utilities.get("animate-bounce").unwrap(), "animation: bounce 1s ease-in-out;");
}
...
fn test_plugin_components() {
    ...
    assert!(components.get(".fade-in-component").unwrap().contains("animation: fade-in"));
    assert!(components.get(".slide-up-component").unwrap().contains("animation: slide-up"));
}
...
fn test_plugin_performance() {
    ...
    for i in 0..10 {
        let mut plugin = AnimationPlugin::new();
        plugin.add_animation(format!("animation-{}", i), 
            format!("@keyframes animation-{} {{ from {{ opacity: 0; }} to {{ opacity: 1; }} }}", i));
    ...
    assert!(duration.as_millis() < 1, "Plugin operations took too long: {:?}", duration);
