...
impl PlaceItemsParser {
    pub fn new() -> Self {
        ...
        place_items_map.insert("place-items-center-safe".to_string(), "safe center".to_string());
    ...
    fn parse_place_items_class(&self, class: &str) -> Option<Vec<CssProperty>> {
        if let Some(place_items_value) = self.place_items_map.get(class) {
            return Some(vec![CssProperty { name: "place-items".to_string(), value: place_items_value.clone(), important: false }]);
        }
        None
    }
}
