...
use tailwind_rs_core::{
    ClassBuilder, ClassValidator, CustomVariant, CustomVariantManager, CustomVariantType,
};
...
fn demo_custom_variants() {
    ...
    let data_theme_dark =
        CustomVariant::data("theme".to_string(), Some("dark".to_string())).unwrap();
    let data_state_loading =
        CustomVariant::data("state".to_string(), Some("loading".to_string())).unwrap();
    ...
    let supports_backdrop_filter =
        CustomVariant::supports("backdrop-filter".to_string(), None).unwrap();

    // Create custom variants
    let custom_theme =
        CustomVariant::custom("theme-mode".to_string(), Some("dark".to_string())).unwrap();

    println!("✅ ARIA Variants:");
    println!(
        "   - {} -> {}",
        aria_checked.to_variant_string(),
        aria_checked.to_css_selector()
    );
    println!(
        "   - {} -> {}",
        aria_disabled.to_variant_string(),
        aria_disabled.to_css_selector()
    );

    println!("✅ Data Variants:");
    println!(
        "   - {} -> {}",
        data_theme_dark.to_variant_string(),
        data_theme_dark.to_css_selector()
    );
    println!(
        "   - {} -> {}",
        data_state_loading.to_variant_string(),
        data_state_loading.to_css_selector()
    );

    println!("✅ Supports Variants:");
    println!(
        "   - {} -> {}",
        supports_grid.to_variant_string(),
        supports_grid.to_css_selector()
    );
    println!(
        "   - {} -> {}",
        supports_backdrop_filter.to_variant_string(),
        supports_backdrop_filter.to_css_selector()
    );

    println!("✅ Custom Variants:");
    println!(
        "   - {} -> {}",
        custom_theme.to_variant_string(),
        custom_theme.to_css_selector()
    );
