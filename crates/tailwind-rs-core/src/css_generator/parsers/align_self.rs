...
impl AlignSelfParser {
    pub fn new() -> Self {
        ...
        self_map.insert("self-baseline-last".to_string(), "last baseline".to_string());
    ...
    fn parse_self_class(&self, class: &str) -> Option<Vec<CssProperty>> {
        if let Some(self_value) = self.self_map.get(class) {
            return Some(vec![CssProperty { name: "align-self".to_string(), value: self_value.clone(), important: false }]);
        }
        None
    }
}
