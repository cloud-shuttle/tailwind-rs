...
impl GridAutoFlowParser {
    pub fn new() -> Self {
        ...
        grid_flow_map.insert("grid-flow-col-dense".to_string(), "column dense".to_string());
    ...
    fn parse_grid_flow_class(&self, class: &str) -> Option<Vec<CssProperty>> {
        if let Some(grid_flow_value) = self.grid_flow_map.get(class) {
            return Some(vec![CssProperty { name: "grid-auto-flow".to_string(), value: grid_flow_value.clone(), important: false }]);
        }
        None
    }
}
