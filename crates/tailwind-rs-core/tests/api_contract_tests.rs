...
fn test_class_builder_api_contract() {
    ...
    assert_eq!(
        custom_props.get("primary-color"),
...
fn test_custom_properties_api() {
    ...
    assert_eq!(
        custom_props.get("--border-radius"),
        Some(&"0.5rem".to_string())
    );
}
...
fn test_complex_class_set_building() {
    ...
    let custom_props = class_set.get_custom_properties();
    assert_eq!(
        custom_props.get("--card-padding"),
        Some(&"1.5rem".to_string())
    );
    assert_eq!(
        custom_props.get("--card-border"),
...
fn test_css_generation_with_complex_class_set() {
    ...
    generator
        .add_responsive_class(Breakpoint::Md, "p-6")
        .unwrap();
    generator
        .add_responsive_class(Breakpoint::Lg, "p-8")
        .unwrap();
...
fn test_performance_with_many_classes() {
    ...
    let valid_spacing = [
        ...
        "p-32", "p-40", "p-48", "p-56", "p-64", "p-72", "p-80", "p-96",
    ];
    let valid_margin = [
        "m-1", "m-2", "m-3", "m-4", "m-5", "m-6", "m-8", "m-10", "m-12", "m-16", "m-20", "m-24",
        "m-32", "m-40", "m-48", "m-56", "m-64", "m-72", "m-80", "m-96",
    ];

    for i in 0..100 {
        generator
            .add_class(valid_spacing[i % valid_spacing.len()])
            .unwrap();
        generator
            .add_class(valid_margin[i % valid_margin.len()])
            .unwrap();
