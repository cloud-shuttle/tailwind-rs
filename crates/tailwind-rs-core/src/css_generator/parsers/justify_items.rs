...
impl JustifyItemsParser {
    pub fn new() -> Self {
        ...
        justify_items_map.insert("justify-items-center-safe".to_string(), "safe center".to_string());
    ...
    fn parse_justify_items_class(&self, class: &str) -> Option<Vec<CssProperty>> {
        if let Some(justify_items_value) = self.justify_items_map.get(class) {
            return Some(vec![CssProperty { name: "justify-items".to_string(), value: justify_items_value.clone(), important: false }]);
        }
        None
    }
}
