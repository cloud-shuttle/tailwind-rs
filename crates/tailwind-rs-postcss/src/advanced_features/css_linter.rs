...
impl CSSLinter {
    ...
    /// Apply linting rule
    fn apply_rule(&self, ast: &CSSAST, rule: &LintRule) -> Result<Vec<LintIssue>, AdvancedFeatureError> {
        let mut issues = Vec::new();
    ...
    fn check_duplicate_selectors(&self, ast: &CSSAST) -> Result<Vec<LintIssue>, AdvancedFeatureError> {
        let mut issues = Vec::new();
        let mut selector_map: HashMap<String, Vec<&CSSRule>> = HashMap::new();
        
        for rule in &ast.rules {
            selector_map.entry(rule.selector.clone()).or_insert_with(Vec::new).push(rule);
        }
    ...
    fn check_important_declarations(&self, ast: &CSSAST) -> Result<Vec<LintIssue>, AdvancedFeatureError> {
        let mut issues = Vec::new();
                ...
                if property.important {
                    issues.push(LintIssue {
                        ...
                        fix: Some(LintFix {
                            ...
                            replacement: property.value.replace("!important", "").trim().to_string(),
                            range: TextRange::new(0, 0),
                        }),
                    });
                }
    ...
    fn check_selector_specificity(&self, ast: &CSSAST, rule: &LintRule) -> Result<Vec<LintIssue>, AdvancedFeatureError> {
        let mut issues = Vec::new();
        let max_specificity = rule.options.get("max")
            .and_then(|v| v.as_u64())
        ...
        for rule in &ast.rules {
            ...
            if specificity > max_specificity {
                issues.push(LintIssue {
                    ...
                    severity: SeverityLevel::Warning,
                    message: format!("Selector '{}' has specificity {} (max: {})", rule.selector, specificity, max_specificity),
                    line: rule.line,
                    column: rule.column,
                    end_line: None,
                    end_column: None,
    ...
    /// Generate suggestions
    fn generate_suggestions(&self, ast: &CSSAST) -> Result<Vec<LintSuggestion>, AdvancedFeatureError> {
    ...
    fn apply_fix(&self, css: &str, fix: &LintFix) -> Result<String, AdvancedFeatureError> {
        match fix.fix_type {
            FixType::Replace => {
                Ok(css.replace(&fix.replacement, ""))
            }
            FixType::Insert => {
                Ok(format!("{}{}", css, fix.replacement))
            }
            FixType::Delete => {
                Ok(css.replace(&fix.replacement, ""))
            }
    ...
    fn get_custom_rule(&self, rule_name: &str) -> Option<&CustomRule> {
        self.config.custom_rules.iter().find(|rule| rule.name == rule_name)
    }
        ...
        vec![
            ...
            LintRule {
                ...
                options: {
                    ...
                    opts.insert("max".to_string(), serde_json::Value::Number(serde_json::Number::from(3)));
                    opts
                },
            },
        ]
...
mod tests {
    ...
    fn test_empty_rules_linting() {
        ...
        let lint_result = result.unwrap();
        assert!(!lint_result.issues.is_empty());
        assert!(lint_result.issues.iter().any(|issue| issue.rule == "no-empty-rules"));
    }
    ...
    fn test_important_linting() {
        ...
        let lint_result = result.unwrap();
        assert!(!lint_result.issues.is_empty());
        assert!(lint_result.issues.iter().any(|issue| issue.rule == "no-important"));
    }
    ...
    fn test_specificity_linting() {
        ...
        let lint_result = result.unwrap();
        assert!(!lint_result.issues.is_empty());
        assert!(lint_result.issues.iter().any(|issue| issue.rule == "selector-max-specificity"));
    }
