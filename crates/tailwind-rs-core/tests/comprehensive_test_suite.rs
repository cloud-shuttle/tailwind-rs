...
mod core_functionality_tests {
    ...
    fn test_basic_layout_classes() {
        ...
        assert!(
            ...
            "CSS should contain display properties"
        );
    }
    ...
    fn test_spacing_classes() {
        ...
        let spacing_classes = vec![
            "p-4", "m-2", "px-3", "py-2", "pt-1", "pr-2", "pb-3", "pl-4", "mx-auto", "my-2",
            "mt-4", "mr-2", "mb-3", "ml-4",
        ];
        ...
        assert!(
            ...
            "CSS should contain padding properties"
        );
        assert!(
            css.contains("margin"),
            "CSS should contain margin properties"
        );
    }
    ...
    fn test_typography_classes() {
        ...
        assert!(
            ...
            "CSS should contain font-size properties"
        );
        assert!(
            css.contains("font-weight"),
            "CSS should contain font-weight properties"
        );
        assert!(
            css.contains("text-align"),
            "CSS should contain text-align properties"
        );
...
mod color_system_tests {
    ...
    fn test_background_colors() {
        ...
        for class in bg_classes {
            ...
            assert!(
                result.is_ok(),
                "Failed to add background color class: {}",
                class
            );
        }
...
mod interactive_states_tests {
    ...
    fn test_active_states() {
        ...
        assert!(
            css.contains(":active"),
            "CSS should contain active selectors"
        );
    }
}
...
mod utility_classes_tests {
    ...
    fn test_effects_and_shadows() {
        ...
        assert!(
            ...
            "CSS should contain box-shadow properties"
        );
        assert!(
            css.contains("opacity"),
            "CSS should contain opacity properties"
        );
    }
    ...
    fn test_backdrop_effects() {
        ...
        assert!(
            ...
            "CSS should contain backdrop-filter properties"
        );
    }
    ...
    fn test_interactive_utilities() {
        ...
        assert!(
            ...
            "CSS should contain pointer-events properties"
        );
        assert!(
            css.contains("cursor"),
            "CSS should contain cursor properties"
        );
        assert!(
            css.contains("user-select"),
            "CSS should contain user-select properties"
        );
    }
}
...
mod edge_cases_tests {
    ...
    fn test_invalid_classes() {
        ...
        for class in invalid_classes {
            ...
            if result.is_ok() {
                ...
                assert!(
                    !css.contains("invalid-class"),
                    "Invalid class should not appear in CSS"
                );
            }
        }
    }
    ...
    fn test_empty_class_set() {
        ...
        assert!(
            css.is_empty() || css.len() < 100,
            "Empty generator should produce minimal CSS"
        );
    }
...
mod performance_tests {
    ...
    fn test_large_class_sets() {
        ...
        let valid_spacing = [
            ...
            "p-24", "p-32", "p-40", "p-48", "p-56", "p-64", "p-72", "p-80", "p-96",
        ];
        let valid_margin = [
            "m-1", "m-2", "m-3", "m-4", "m-5", "m-6", "m-8", "m-10", "m-12", "m-16", "m-20",
            "m-24", "m-32", "m-40", "m-48", "m-56", "m-64", "m-72", "m-80", "m-96",
        ];
        let valid_text = [
            "text-xs",
            "text-sm",
        ...
        for i in 0..100 {
            classes.push(valid_spacing[i % valid_spacing.len()].to_string());
            classes.push(valid_margin[i % valid_margin.len()].to_string());
            classes.push(valid_text[i % valid_text.len()].to_string());
            classes.push(valid_bg[i % valid_bg.len()].to_string());
        }
    ...
    fn test_memory_usage() {
        ...
        // Add many classes and check memory usage using valid classes
        let valid_classes = [
            "p-1",
            "p-2",
            "p-3",
        ...
        for i in 0..1000 {
            generator
                .add_class(valid_classes[i % valid_classes.len()])
                .unwrap();
        }
...
mod integration_tests {
    ...
    fn test_comprehensive_css_generation() {
        ...
        assert!(
            result.is_ok(),
            "Comprehensive CSS generation should succeed"
        );
...
mod regression_tests {
    ...
    fn test_previously_broken_classes() {
        ...
        for class in previously_broken {
            ...
            assert!(
                result.is_ok(),
                "Previously broken class should now work: {}",
                class
            );
        }
    ...
    fn test_api_consistency() {
        ...
        assert!(
            css1.contains("bg-blue-500"),
            "CSS1 should contain bg-blue-500"
        );
        assert!(
            css1.contains("text-white"),
            "CSS1 should contain text-white"
        );
        assert!(css1.contains("p-4"), "CSS1 should contain p-4");

        assert!(
            css2.contains("bg-blue-500"),
            "CSS2 should contain bg-blue-500"
        );
        assert!(
            css2.contains("text-white"),
            "CSS2 should contain text-white"
        );
