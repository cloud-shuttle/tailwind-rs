...
impl std::fmt::Display for ContractError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ContractError::BackwardCompatibilityViolation(msg) => write!(f, "Backward compatibility violation: {}", msg),
        }
    }
}
...
impl ApiContract for ClassBuilderContract {
    ...
    fn validate_input(&self, input: &Self::Input) -> Result<(), ContractError> {
        ...
        for class in &input.classes {
            ...
            if class.contains(" ") {
                return Err(ContractError::InvalidInput("Class name contains spaces".to_string()));
            }
        }
        ...
        for (breakpoint, _) in &input.responsive {
            match breakpoint {
                ...
                Breakpoint::Lg | Breakpoint::Xl | Breakpoint::Xl2 => {},
                _ => return Err(ContractError::InvalidInput("Invalid breakpoint".to_string())),
            }
        }
    ...
    fn validate_output(&self, output: &Self::Output) -> Result<(), ContractError> {
        ...
        if output.len() == 0 && !output.is_empty() {
            return Err(ContractError::InvalidOutput("Invalid ClassSet state".to_string()));
        }
        
        // Validate CSS class string format
        let css_classes = output.to_css_classes();
        if css_classes.contains("  ") {
            return Err(ContractError::InvalidOutput("CSS classes contain double spaces".to_string()));
        }
...
impl ContractValidator {
    ...
    pub fn validate_call<T>(&self, api_name: &str, input: T) -> Result<(), ContractError> {
        ...
        if let Some(contract) = self.contracts.get(api_name) {
            ...
            Err(ContractError::ContractViolation(format!("Unknown API: {}", api_name)))
        }
    }
