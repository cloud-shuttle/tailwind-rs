...
mod text_shadow_performance_tests {
    ...
    fn test_text_shadow_generation_performance() {
        ...
        for _ in 0..1000 {
            let _ = ClassBuilder::new().text_shadow_sm().build();
        }

        let duration = start.elapsed();
        assert!(
            duration.as_millis() < 50,
            "Text shadow generation too slow: {}ms",
            duration.as_millis()
        );
    }
