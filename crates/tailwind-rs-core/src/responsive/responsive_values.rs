...
impl<T> ResponsiveValue<T> {
    ...
    pub fn get_breakpoint_or_base(&self, breakpoint: Breakpoint) -> Option<&T> {
        self.values
            .get(&breakpoint)
            .or_else(|| self.values.get(&Breakpoint::Base))
    }
    ...
    pub fn get_for_width(&self, screen_width: u32) -> Option<&T> {
        ...
        let active_breakpoints: Vec<Breakpoint> = self
            .values
            .keys()
...
mod tests {
    ...
    fn test_responsive_value_set_get_breakpoint() {
        ...
        assert_eq!(
            rv.get_breakpoint(Breakpoint::Base),
            Some(&"base".to_string())
        );
    ...
    fn test_responsive_value_get_breakpoint_or_base() {
        ...
        assert_eq!(
            rv.get_breakpoint_or_base(Breakpoint::Base),
            Some(&"base".to_string())
        );
        assert_eq!(
            rv.get_breakpoint_or_base(Breakpoint::Sm),
            Some(&"sm".to_string())
        );
        assert_eq!(
            rv.get_breakpoint_or_base(Breakpoint::Md),
            Some(&"base".to_string())
        );
    }
