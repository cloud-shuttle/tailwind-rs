    ...
    fn prefix_rule(&self, rule: &str, browsers: &[String]) -> Result<String, AutoprefixerError> {
        ...
        for line in rule.lines() {
            if line.trim().ends_with('{') {
                ...
                if in_declaration {
                    let prefixed_declaration =
                        self.prefix_declaration(&current_declaration, browsers)?;
...
mod tests {
    ...
    fn test_basic_prefixing() {
        ...
        assert!(
            prefixed.len() > css.len()
                || prefixed.contains("-webkit-")
                || prefixed.contains("-moz-")
                || prefixed.contains("-ms-")
        );
    }
    ...
    fn test_prefix_generator() {
        ...
        let prefixes =
            generator.generate_prefixes("display", &["chrome".to_string(), "firefox".to_string()]);
