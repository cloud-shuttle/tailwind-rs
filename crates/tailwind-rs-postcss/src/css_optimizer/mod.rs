...
impl CSSOptimizer {
    ...
    pub fn optimize(&mut self, css: &str) -> Result<types::OptimizationResult, types::OptimizationError> {
        ...
        if self.config.optimize_properties {
            let property_result = self.property_optimizer.optimize_properties(&optimized_css)?;
