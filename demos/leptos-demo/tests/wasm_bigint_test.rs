...
mod wasm_bigint_tests {
    ...
    fn test_wasm_bigint_error_reproduction() {
        ...
        assert!(stderr.contains("WASM_BIGINT") || output.status.success(), 
                "Expected WASM_BIGINT error, but got: {}", stderr);
    }
    ...
    fn test_alternative_linker_flags() {
        let test_cases = vec![
            vec!["build", "--target", "web", "--out-dir", "pkg-test2", "--", "--no-default-features"],
            vec!["build", "--target", "web", "--out-dir", "pkg-test3", "--", "--features", "console_error_panic_hook"],
            vec!["build", "--target", "web", "--out-dir", "pkg-test4", "--", "--release"],
        ];
...
"#;
        ...
        std::fs::write("Cargo-minimal.toml", minimal_cargo).expect("Failed to write minimal Cargo.toml");
        ...
        if output.status.success() {
            ...
            println!("FAILED: Even minimal test fails - issue is with toolchain: {}", stderr);
        }
    }
